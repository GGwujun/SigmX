...
export function Agent() {
  ...
  useEffect(() => {
    onStatusChange((s) => {
      act().setSseStatus(s);
      if (s === "reconnecting" && prevSseStatusRef.current === "connected") toast.warning("连接断开，重新连接中…");
  ...
  const runPrompt = async (prompt: string) => {
    ...
    if (goalComposerActive) {
      ...
      try {
        ...
        act().setStatus("idle");
        toast.error(error instanceof Error ? error.message : "启动目标失败");
      }
      return;
    }
