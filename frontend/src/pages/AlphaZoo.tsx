...
const ZOO_CARDS: ZooCard[] = [
  ...
  {
    id: "alpha101",
    title: "Kakushadze 101 公式化 Alpha",
    description:
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-wide">
          <Layers className="h-3.5 w-3.5" aria-hidden="true" /> 因子工厂
        </div>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">
          {total > 0 ? total : 452} 个预构建量化 Alpha，涵盖 4 个因子库
        </h1>
        <p className="text-sm text-muted-foreground max-w-2xl">
          浏览来自 Qlib、Kakushadze 101、GTJA 191 和学术异象文献的公式化截面信号。
          点击任意 Alpha 查看公式与源码，或运行基准测试对整个因子库进行评分。
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-3 border rounded-xl p-4 bg-card">
        ...
        <Link
          ...
          title="勾选 2 个以上 Alpha，然后进行一对一对比"
        >
      ...
      <div className="border rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm" aria-label="Alpha 因子目录">
            <caption className="sr-only">Alpha 因子目录</caption>
            <thead>
  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center text-muted-foreground">
        <Loader2 className="h-4 w-4 animate-spin mr-2" aria-hidden="true" /> 加载 {alphaId} 中…
      </div>
    );
  }
        ...
        <div className="border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <tbody>
              ...
              <MetaRow label="最小预热期数" value={metaString(meta, "min_warmup_bars")} />
      ...
      <section className="space-y-2">
        ...
        <details className="border rounded-xl bg-card group">
          ...
          <pre className="border-t bg-muted/30 p-4 overflow-x-auto text-xs leading-relaxed">
            <code>{detail.source_code || "（无源码可用）"}</code>
          </pre>
        </details>
      </section>
    </div>
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground max-w-2xl">
          计算所选因子库中每个 Alpha 在指定市场和周期上的 IC / IR，并将其分类为有效 / 反转 / 失效。
        </p>
      </div>
      ...
      >
        ...
        <div>
          <label htmlFor="bench-period" className="text-xs text-muted-foreground block mb-1">时间区间</label>
          <input
            id="bench-period"
            value={period}
        ...
        <div>
          <label htmlFor="bench-top" className="text-xs text-muted-foreground block mb-1">排名数</label>
          <input
            id="bench-top"
            type="number"
            min={1}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <TopTable title="IR 排名前 5" rows={result.top5_by_ir || []} />
        <TopTable title="反转最显著" rows={(result.dead_examples || []).slice(0, 3)} />
      </div>
...
function CompareView() {
  ...
  const startCompare = async (e: FormEvent) => {
    ...
    if (status === "submitting" || status === "streaming") return;
    if (ids.length < 2) {
      setFormError("请输入至少 2 个不同的 Alpha ID 进行对比。");
      return;
    }
    setStatus("submitting");
    setProgress(null);
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-wide">
          <ArrowLeftRight className="h-3.5 w-3.5" aria-hidden="true" /> 一对一对比
        </div>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">
          并排对比 Alpha 因子
        </h1>
        <p className="text-sm text-muted-foreground max-w-2xl">
          仅对您挑选的 Alpha 在指定市场和周期上进行基准测试，按 IC / IR 排名并显示与最优值的差距——当您只需关注短名单时远比全库基准测试更快。
        </p>
      </div>
      ...
      <form onSubmit={startCompare} className="border rounded-xl p-4 bg-card space-y-3">
        <div>
          <label htmlFor="compare-ids" className="text-xs text-muted-foreground block mb-1">
            Alpha ID{ids.length > 0 ? `（已选 ${ids.length} 个）` : ""}
          </label>
          <textarea
            id="compare-ids"
            value={idsText}
          ...
          <p className="text-[11px] text-muted-foreground mt-1">
            用逗号或空格分隔 ID。提示：在目录中勾选 Alpha 后点击「对比」即可自动填充。
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            <label htmlFor="compare-period" className="text-xs text-muted-foreground block mb-1">时间区间</label>
            <input
              id="compare-period"
              value={period}
              onChange={(e) => setPeriod(e.target.value)}
          ...
          <div>
            <label htmlFor="compare-sort" className="text-xs text-muted-foreground block mb-1">排序方式</label>
            <select
              id="compare-sort"
              value={sort}
        ...
        <div className="flex items-center gap-3">
          ...
          {ids.length < 2 && (
            <span className="text-xs text-muted-foreground">请至少选择 2 个 Alpha。</span>
          )}
        </div>
...
function CompareResultPanel({ result }: { result: AlphaCompareResult }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm" aria-label="Alpha 对比排名">
            <thead>
              <tr className="border-b bg-muted/40 text-muted-foreground text-xs">
                ...
                <th className="text-right px-3 py-2 hidden md:table-cell" title="IC>0 的周期占比">IC&gt;0</th>
                <th className="text-right px-3 py-2 hidden lg:table-cell" title="IC 样本数">n</th>
                <th className="text-right px-3 py-2" title={`与最优值在 ${result.sort} 上的差距`}>Δ {result.sort}</th>
              </tr>
            </thead>
            <tbody>
