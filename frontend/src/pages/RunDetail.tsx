...
export function RunDetail() {
  ...
  const TABS: { id: Tab; label: string; icon: typeof BarChart3; hidden?: boolean }[] = [
    ...
    { id: "trades", label: "交易记录", icon: List },
    { id: "validation", label: "统计验证", icon: ShieldCheck, hidden: !hasValidation },
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="grid gap-3 md:grid-cols-4">
        <RunCardStat label="版本" value={card.schema_version || "未知"} />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <RunCardPanel title="可复现性" icon={Fingerprint}>
          <KeyValueTable data={reproducibility} empty="未记录可复现哈希" monospaceValues />
        </RunCardPanel>
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <RunCardPanel title="指标" icon={BarChart3}>
          <KeyValueTable data={metrics} empty="未记录指标数据" />
        </RunCardPanel>
