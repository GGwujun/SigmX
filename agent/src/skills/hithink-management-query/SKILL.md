...
默认且优先使用 SigmX Data Hub；只在清单声明允许且指标口径一致时使用候补源。 `python -m src.skill_runtime.cli stocks.holder_num --params '<JSON>'`
...
```bash
python -m src.skill_runtime.cli stocks.holder_num --params '{}'
```
